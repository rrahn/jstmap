...
TEST(jstmap_simulate, simulate_alignment)
{
    ...
    jstmap::aligned_sequence_t aligned;
    seqan3::assign_unaligned(aligned, reference);
    EXPECT_EQ(aligned, stripped_reference);
}
...
TEST(jstmap_simulate, simulate_alignment_negative_error_rate)
{
    std::filesystem::path sequence_file{DATADIR"sim_reads_ref1x10.fa"};
    jstmap::sequence_t reference = jstmap::load_reference(sequence_file);
    EXPECT_THROW(jstmap::simulate_alignment(reference, -1), std::runtime_error);
}
...
TEST(jstmap_simulate, simulate_alignment_too_high_error_rate)
{
    std::filesystem::path sequence_file{DATADIR"sim_reads_ref1x10.fa"};
    jstmap::sequence_t reference = jstmap::load_reference(sequence_file);
    EXPECT_THROW(jstmap::simulate_alignment(reference, 2), std::runtime_error);
}
